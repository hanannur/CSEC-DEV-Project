...
const LoginPage: React.FC<Props> = ({ onLogin, onNavigate }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    steps.forEach((step, index) => {
      setTimeout(() => setAuthStatus(step), index * 600);
    });
  ...
  // Keeping provider login as simulation for now as social auth is not yet implemented on backend
  const simulateSocialLogin = (selectedProvider: 'google' | 'github') => {
    // ... logic remains similar or we can wait for backend ...
  };
